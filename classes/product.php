	...
	*/
	class product
	{
		...
		public function insert_product($date,$files){
			...
			if($product_code ='' || $productName == "" || $productQuantity == "" || $category == "" || $brand == "" || $product_desc == "" || $price == "" || $file_name == ""){
				$alert = "<span class='error'>Các trường không được để trống!</span>";
				return $alert;
			}else{
				move_uploaded_file($file_temp, $uploaded_image);

				$query = "INSERT INTO tbl_product(productName,product_code,product_remain,productQuantity,catId,brandId,product_desc,price,image) VALUES('$productName','$product_code','$productQuantity','$productQuantity','$category','$brand','$product_desc','$price','$unique_image') ";
				$result = $this->db->insert($query);
				if($result){
					$alert = "<span class='success'>Chỉnh sửa sản phẩm thành công</span>";
					return $alert;
				}else {
					$alert = "<span class='error'>Chỉnh sửa sản phẩm KHÔNG thành công</span>";
					return $alert;
				}
			}
		}
			...
			if($sliderName=="" || $type==""){
				...
				if(!empty($file_name)){
					...
					if ($file_size > 2048000) {
		    		 ...
		    		 $alert = "<span class='success'>Kích cỡ ảnh nên nhỏ hơn 10MB!</span>";
					return $alert;
				    } 
					...
					$result = $this->db->insert($query);
					if($result){
						$alert = "<span class='success'>Thanh trượt đã thêm vào thành công</span>";
						return $alert;
					}else {
						$alert = "<span class='error'>Thanh trượt đã thêm vào KHÔNG thành công</span>";
						return $alert;
					}
				}
		...
		public function del_slider($id)
		{
			...
			if($result){
				$alert = "<span class='success'>Thanh trượt đã xóa thành công</span>";
				return $alert;
			}else {
				$alert = "<span class='success'>Thanh trượt đã xóa KHÔNG thành công</span>";
				return $alert;
			}
		}
		...
		public function update_product($data,$files,$id){
			...
			if($product_code == "" || $productName=="" || $productQuantity=="" || $brand=="" || $category=="" || $product_desc=="" || $price=="" ){
				$alert = "<span class='error'>Các trường không để trống!</span>";
				return $alert; 
			}else{
				if(!empty($file_name)){
					//Nếu người dùng chọn ảnh
